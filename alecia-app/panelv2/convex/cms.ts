...
export const voteOnProposal = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (user.role !== "partner" && user.role !== "sudo") throw new Error("Unauthorized to vote");
    ...
    const cleanVotesAgainst = proposal.votesAgainst.filter((id) => id !== user._id);
...
export const mergeProposal = mutation({
  ...
  handler: async (ctx, args) => {
    const user = await getAuthenticatedUser(ctx);
    if (user.role !== "sudo" && user.role !== "partner") throw new Error("Unauthorized");
    ...
    const partners = await ctx.db.query("users")
        .filter(q => q.or(q.eq(q.field("role"), "partner"), q.eq(q.field("role"), "sudo")))
        .collect();
    ...
    if (approvalRate < quorumPercent) {
        throw new Error(`Quorum not met. Current: ${approvalRate.toFixed(1)}%, Required: ${quorumPercent}%`);
    }
