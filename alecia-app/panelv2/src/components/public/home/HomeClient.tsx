...
import { ValuationEstimator } from "@/components/public/features/valuation-estimator";
import { HeroBackground } from "@/components/public/features/hero-background";
import { ExpertiseCard } from "@/components/public/features/expertise-card";
import { NewsletterForm } from "@/components/public/features/newsletter-form";
import { TeamMemberModal } from "@/components/public/features/team-member-modal";
...
export function HomeClient() {
  ...
  // CMS Integration
  const pageData = useQuery(api.cms.getPage, { slug: "home" });
  ...
  // Helper to extract text from HTML content (simple regex for MVP)
  const extractContent = (html: string | undefined, tag: string) => {
      if (!html) return null;
  ...
  const heroTitle = extractContent(pageData?.content, "h1") || t("hero.titlePart1") + " " + t("hero.titlePart2");
  const heroSubtitle = extractContent(pageData?.content, "p") || t("hero.subtitle");
        ...
        <section className="relative flex flex-col items-center justify-center min-h-[90vh] px-6 py-20 pt-32 text-center overflow-hidden">
          ...
          >
            ...
            <h1 className="font-[family-name:var(--font-playfair)] text-5xl md:text-7xl font-semibold text-[var(--foreground)] mb-6 leading-tight"
                dangerouslySetInnerHTML={{ __html: heroTitle }} // Allow HTML for formatting if needed
            />
          ...
          {/* Scroll indicator - enhanced with chevrons and fade-out */}
          <motion.div
