...
export function FilesTab() {
  ...
  return (
    <div className="space-y-6">
      {/* Upload Zone */}
      <div className="bg-white dark:bg-boxdark border border-stroke dark:border-strokedark rounded-xl overflow-hidden shadow-sm">
        <div className="p-6.5">
          <div
            {...getRootProps()}
          ...
          {uploadingFiles.length > 0 && (
            <div className="mt-4 space-y-2">
              {uploadingFiles.map((file, idx) => (
                <div key={idx} className="flex items-center gap-3 p-3 rounded-lg bg-gray-2 dark:bg-meta-4">
      ...
      <div className="bg-white dark:bg-boxdark border border-stroke dark:border-strokedark rounded-xl overflow-hidden shadow-sm">
        <div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark">
          <h3 className="font-medium text-black dark:text-white">Fichiers récents</h3>
        </div>
        <div className="flex flex-col">
          {mockDocuments.map((doc) => {
            const FileIcon = getFileIcon(doc.mimeType);
            return (
              <div key={doc.id} className="flex items-center justify-between gap-4 px-6 py-4 border-b border-stroke dark:border-strokedark last:border-0 hover:bg-gray-2 dark:hover:bg-meta-4 transition-colors">
                <div className="flex items-center gap-4 min-w-0">
                    <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                      <FileIcon className="w-5 h-5 text-primary" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-black dark:text-white font-medium truncate">{doc.name}</p>
                ...
                <div className="flex items-center gap-2">
                  <span className={`p-1.5 rounded-md ${doc.isConfidential ? "bg-danger/10 text-danger" : "bg-success/10 text-success"}`}>
                    {doc.isConfidential ? <Lock className="w-4 h-4" /> : <Unlock className="w-4 h-4" />}
                  </span>
                  <button className="flex items-center gap-1 px-3 py-1.5 rounded border border-stroke dark:border-strokedark hover:text-primary text-sm" onClick={() => copyMagicLink(doc.id)}>
                    {copiedId === doc.id ? <><Check className="w-4 h-4 text-success" /><span className="hidden sm:inline text-success">Copié</span></> : <><Link2 className="w-4 h-4" /><span className="hidden sm:inline">Partager</span></>}
                  </button>
                  <button className="p-1.5 hover:text-primary"><Download className="w-4 h-4" /></button>
                  <button className="p-1.5 hover:text-danger"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            );
          })}
