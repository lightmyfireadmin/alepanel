      ...
      <Card className="col-span-1 md:col-span-2 xl:col-span-3">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="ghost" size="sm" asChild>
            <Link href="/admin/forum" className="text-primary flex items-center gap-1">
        ...
        <CardContent>
            <div className="space-y-4">
                {recentThreads.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">Aucune discussion récente.</p>
                ) : (
                    recentThreads.map((thread) => (
                        <Link 
                            key={thread.id} 
                            href={`/admin/forum/thread/${thread.id}`}
                            className="flex items-center justify-between border-b border-stroke dark:border-strokedark pb-3 last:border-0 last:pb-0 hover:bg-gray-50 dark:hover:bg-meta-4/10 px-2 -mx-2 rounded transition-colors"
                        >
                            <div className="flex items-center gap-3 min-w-0">
                                <div className="h-9 w-9 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)] shrink-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-xs text-bodydark2 flex items-center gap-1">
                                        <span className="font-medium text-[var(--accent)]">{thread.categoryName}</span>
                                        <span>•</span>
                                        <span>Par {thread.authorName}</span>
                                    </p>
                                </div>
                            </div>
                            <div className="text-xs text-bodydark2 shrink-0 flex items-center gap-1 ml-4">
